...
load("sf_network/hourly_centrality_stdnet.rdata")
```
...
Preparing a tidy data frame with all the centrality results. Relative difference in
centrality are adjusted by $n - 1$, where $n$ is the number of nodes in the component.
This is done because BC is the number of shortest paths from/to anytother nodes in the network,
however, in our case, we are also interested in the paths from/to that particular link.
