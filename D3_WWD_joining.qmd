...
A subset of the reports during the morning peak hour +/- 2 hours
...
Fitting GLM for the different parameters
...
ggplot(aes(x = wwd.speed,
  ...
  scale_size_binned(transform = "log10")
```
...
ggplot(aes(x = dist.th,
  ...
  theme_minimal()+
  scale_color_viridis_b(option = "plasma")+
  scale_size_binned(transform = "log")
```
