...
setnames(summary_speeds,
         old = c("Min.","1st Qu.","Median","Mean","3rd Qu.","Max."),
         new = c("d_min_speed","d_q1_speed","d_median_speed","d_mean_speed","d_q3_speed","d_max_speed"))
...
```{r}
norm_summary_spd |> 
  ggplot(aes(x = HORA,y = d_norm_speed))+
...
```{r}
norm_summary_spd |> 
  ggplot(aes(x = HORA,y = d_median_speed))+
  geom_boxplot(aes(group = HORA), fill = NA,alpha = 0.3,outlier.shape = NA)+
  geom_jitter(alpha = 0.3, size = 0.3,col = "gray60")+
  stat_summary(geom = "line",
               fun = "mean",
...
st_write(sf_speed,dsn = "sf_network/sf_speed_network.gpkg",delete_dsn = T)
