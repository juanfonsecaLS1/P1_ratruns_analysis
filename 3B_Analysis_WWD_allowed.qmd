...
st_write(simpl_network_sf,dsn = "sf_network/simplified_network.gpkg",append = F)
```
...
sf_buffer_sample <- tibble(id = "point",
                    lat = 4.636790742899646,
                    lon = -74.06624384801985) |> 
  st_as_sf(coords = c("lon","lat"),crs = 4326) |> 
  st_transform(st_crs(major_net)) |> 
  st_buffer(dist = 200,endCapStyle = "SQUARE")

major_net_sample <- major_net[sf_buffer_sample,]

major_buffer_sample <- major_net_sample |> st_buffer(15)



subset_sample <- simpl_subset_sf[sf_buffer_sample,]

subset_buffer_sample <- subset_sample |> st_buffer(20,endCapStyle = "FLAT")

sf_off_sample <- bind_rows(
  off_sf_all[sf_buffer_sample,]|>
    filter(year == 2019,
           day_type == "weekday") |> 
    mutate(type = "Not assigned"),
  minor_offences[sf_buffer_sample,]|>
    filter(year == 2019,
           day_type == "weekday") |> 
    mutate(type = "Assigned to minor"))



minor_offences_sample <- minor_offences[sf_buffer_sample,] |>
    filter(year == 2019,
           day_type == "weekday")


minor_offences_sample$near_index <- st_nearest_feature(minor_offences_sample,subset_sample)
minor_offences_sample$pair_id <- subset_sample$pair_id[minor_offences_sample$near_index]

sf_lines_nearest <- st_nearest_points(subset_sample[minor_offences_sample$near_index,],minor_offences_sample,pairwise = T)

# tmap_mode("plot")

map_assignment_sample <- tm_shape(major_net_sample,
                                  bbox = st_bbox(sf_buffer_sample),
                                  relative = T,
                                  xlim = c(-.6,1.1),
                                  ext = 1.5,
                                  unit = "m")+
  tm_lines("gray60",col_alpha = 0.8,lwd = 4)+
  tm_shape(major_buffer_sample)+
  tm_fill("gray60",fill_alpha = 0.3)+
  tm_shape(subset_sample)+
  tm_lines("dodgerblue3",col_alpha = 0.8,lwd = 2)+
  tm_shape(subset_buffer_sample)+
  tm_fill("dodgerblue2",fill_alpha = 0.3)+
  tm_shape(sf_off_sample)+
  tm_dots(fill = "type",
          fill.scale = tm_scale(values =c("firebrick2","goldenrod")),
          fill.legend = tm_legend(title = "Offence Reports",position = tm_pos_in(),frame = F,bg.alpha = 0),
          fill_alpha = "type",
          fill_alpha.scale = tm_scale(values = c(0.8,0.3)),
          fill_alpha.legend = NULL)+
  tm_shape(sf_lines_nearest)+
  tm_lines("black",col_alpha = 0.6)+
  tm_scalebar(position = c("left","bottom"),breaks = c(0,50,100,150,200))+
  tm_add_legend(
    type = "lines",  
    labels = c("Major", "Minor"),
    col = c("gray60", "dodgerblue3"),
    lwd = 2,
    title = "Road type",position = tm_pos_in()
  )

tmap_save(tm = map_assignment_sample,filename = "map_tm_off_assigment.png",units = "cm",height = 10,width = 12)
...
slopes_sector <- plot_slopes(AIMc,
            variables = "logdiff_max",
