...
bog_sfnet <- bog_sfnet |>
  activate("edges") |> 
  mutate(cent_sfn_time = tidygraph::centrality_edge_betweenness(weights = time_weighted, directed = T))
...
bog_sfnet |> 
  activate("edges") |>
  data.frame() |> 
  select(cent_time,cent_sfn_time) |> 
  pairs()
```
...
bog_sfnet |> 
  activate("edges") |>
  data.frame() |> 
  with(identical(cent_time,cent_sfn_time))
```
